(function(factory) {
	...
	* @param {String|RegExp} [endRE=/\s+$/gm || startRE] RegExp match to trim from the end of the string
	...
	*/
	...
	/**
	* See if any of the elements in "testArray" match the compairison arguments provided.
	*
	* @parent devoir.utils
	* @method ifAny
					...
					if (match && match.index === offset) {
						...
						if (!(match instanceof Tokenizer.prototype.SKIP)) {
							if (success instanceof Function) {
								...
								if (!(ret instanceof Object) || root.indexOf(ret, 'string', 'number', 'array', 'function', 'boolean')) {
