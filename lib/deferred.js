(function(factory) {
	...
	* @param {Object} {[options]} Options object. There are two possible options:<br>
	* * "raw": (default false), if true, deferred resolution arguments will be passed as a single array to any bound resolution / rejection listeners. By default resolution arguments will be passed directly to any bound listeners in the order provided to "resolve"
	* * "immediate": (default false), if true, this deferred will call its callback immediately, and also call any listeners immediately upon resolution / rejection. Default is to call callback and listeners on "nextTick"
	...
	**/
	...
	function Deferred(cb, _opts) {
		...
		/**
		* Call *successCallback* when deferred resolves (or *errorCallback* if deferred is rejected), passing arguments as provided to @@devoir.Deferred.resolve (unless the "raw" option is set, and then all arguments will be provided in a single array)
		*
		* @parent devoir.Deferred
		* @function then
		...
		* @return {Deferred} Return a new deferred that can be used for chaining (if *callback* returns a deferred, this deferred wont resolve until the one returned by *callback* resolves) 
		**/
		newROProperty('then', then);

		/**
		* This is an alias to @@devoir.Deferred.then
		*
		* @parent devoir.Deferred
		* @function done
		* @see devoir.Deferred.then
		**/
		newROProperty('done', then);

		/**
		* Call *errorCallback* if deferred is rejected. This is an alias for @@devoir.Deferred.fail
		*
		* @parent devoir.Deferred
		* @function catch
		...
		**/
		...
		/**
		* Call *errorCallback* if deferred is rejected
		*
		* @parent devoir.Deferred
		* @function fail
		* @param {Function} {errorCallback} Callback function to call if deferred is rejected
		...
		**/
		...
		/**
		* Call *notificationCallback* if deferred gets a notification event
		*
		* @parent devoir.Deferred
		...
		**/
		...
		/**
		* Call *callback* if deferred gets resolved or rejected. Call *notificationCallback* if deferred gets a progress notification
		*
		* @parent devoir.Deferred
		* @function always
		...
		**/
		...
		/**
		* Resolve this deferred with the arguments provided
		*
		* @parent devoir.Deferred
		* @function resolve
		...
		**/
		...
		/**
		* Reject this deferred with the arguments provided
		*
		* @parent devoir.Deferred
		* @function reject
		...
		**/
		...
		/**
		* Notify this deferred with a progress update (usually a Number between 0 and 1, but is not required to be a number)
		*
		* @parent devoir.Deferred
		* @function notify
		...
		**/
		...
		/**
		* Return a special clone deferred object that can be used as a normal "view" into this deferred, but can not be updated / modified (read only)
		*
		* @parent devoir.Deferred
		* @function promise
		...
		**/
		...
		/**
		* Get the status of this deferred. Can be one of "pending", "fulfilled", or "rejected"
		*
		* @parent devoir.Deferred
		* @function status
		...
		**/
		...
		/**
		* Alias for @@devoir.Deferred.status
		*
		* @parent devoir.Deferred
		* @function state
		* @return {String} "pending" if the deferred is still pending, "fulfilled" if the deferred has been resolved, or "rejected" if the deferred has been rejected
		* @see devoir.Deferred.status
		**/
		...
		/**
		* Get the progress for this deferred as set by notifications
		*
		* @parent devoir.Deferred
		* @function progress
		* @return {*} The current progress (usually a Number) of this deferred
		* @see devoir.Deferred.notify
		**/
		...
		/**
		* Proxy the resolution / rejection / notifications of this deferred to the deferred specified by *deferred* argument
		*
		* @parent devoir.Deferred
		* @function proxy
		* @param {Deferred} {deferred} The deferred to proxy events to
		* @return {Deferred} A new Deferred that can be used for chaining
		* @see devoir.Deferred.resolve
		* @see devoir.Deferred.reject
		* @see devoir.Deferred.notify
		**/
		...
		/**
		* Get/Set this deferred's "immediate" mode. In "immediate" mode all callbacks are called immediately, instead of "nextTick", which is the default
		*
		* @parent devoir.Deferred
		* @function immediate
		...
		* @return {Deferred|Boolean} If *set* argument is specified, *this* is returned. If *set* is not specified, return the current "immediate" mode state
		**/
		...
		/**
		* Get/Set this deferred's "raw" mode. In "raw" mode all callbacks are called with a single array of arguments as the argument to any callbacks instead of passing arguments as supplied to resolve/reject/notify
		*
		* @parent devoir.Deferred
		* @function raw
		* @param {Boolean} {[set]} If specified, set "raw" mode according to boolean value. If not specified, return "raw" mode state
		* @return {Deferred|Boolean} If *set* argument is specified, *this* is returned. If *set* is not specified, return the current "raw" mode state
		**/
	...
	/**
	* This will create a new deferred that waits on the status of ALL deferreds passed to it. If any one of the deferreds is rejected the entire chain is rejected
	*
	* @parent devoir.Deferred
	* @function all
	* @static
	* @param {Deferred} {deferreds} Array if deferreds to wait on
	...
	**/
	...
	/**
	* This will create a new deferred that waits on the status of ALL deferreds passed to it. This deferred will be resolved/rejected as soon as any one of the deferreds is resolved or rejected
	*
	* @parent devoir.Deferred
	* @function race
	* @static
	...
	**/
	...
	/**
	* This will create a new deferred that waits on the status of ALL deferreds passed to it. This deferred will not resolve / reject until ALL deferreds have resolved / rejected. If anyone of the deferreds has been rejected than the whole chain is rejected
	*
	* @parent devoir.Deferred
	* @function every
	* @static
	...
	**/
	...
	/**
	* Create an immediately resolved deferred with any arguments passed to resolve. This is the same as: `var def = new devoir.Deferred(undefined, {immediate: true});def.resolve(args...);`
	*
	* @parent devoir.Deferred
	* @function resolve
	* @static
	...
	**/
	...
	/**
	* Create an immediately rejected deferred with any arguments passed to reject. This is the same as: `var def = new devoir.Deferred(undefined, {immediate: true});def.reject(args...);`
	*
	* @parent devoir.Deferred
	* @function reject
	* @static
